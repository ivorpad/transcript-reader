...
The first six show a fictional session generated by `npm run sample`: a flaky
queue spec, three agents, one interrupt, a spilled tool result and an 18-hour
gap. The last is a real 104-line SDK run, filtered to its trouble.

![The session opened as bands, one per turn](docs/screenshots/01-session-bands.png)

![Rows in full: a failed Bash call with its output, its folded hooks and the inspector](docs/screenshots/02-session-rows.png)

![Agent calls and an Edit drawn as a diff](docs/screenshots/03-session-agents-and-diff.png)

![The fleet: lanes against active time, then the tree](docs/screenshots/04-fleet.png)

![Folded: every line that drew no row, and the row it attached to](docs/screenshots/05-folded.png)

![The catalog of five classes with this session's counts](docs/screenshots/06-catalog.png)

![Only the trouble: a short SDK run filtered to its three failed Bash calls, each open, one selected in the inspector](docs/screenshots/07-trouble.png)
