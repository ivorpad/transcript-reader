...
A short SDK run, filtered to its trouble: three failed Bash calls, each open,
one selected in the inspector.

![Only the trouble: a short SDK run filtered to its three failed Bash calls, each open, one selected in the inspector](docs/screenshots/trouble.png)
...
npm run sample       # regenerate the fictional session under docs/sample
